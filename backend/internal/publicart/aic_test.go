...
func TestAICProviderSearchBuildsCandidatesWithIIIFImageURLs(t *testing.T) {
	...
	wantThumbnailURL := "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=="
	...
	if got.ThumbnailURL == got.ImageURL {
		t.Fatal("ThumbnailURL should be a fallback preview source, not the full preview image")
	}
